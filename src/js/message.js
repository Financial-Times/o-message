	...
	constructMessageElement () {
		if (this.opts.typeNucleus === 'alert') {
		 	if (!this.opts.content.highlight) {
				throwError(`\nAn ${this.opts.typeNucleus} message element requires 'options.content.highlight'`);
			} else {
				return construct.alertMessage(this.opts);
			}
		} else if (this.opts.typeNucleus === 'notice') {
			if (!this.opts.content.detail) {
				throwError(`\nAn ${this.opts.typeNucleus} message element requires 'options.content.detail'`);
			} else {
				return construct.noticeMessage(this.opts);
			}
		} else {
			throwError(`\n'${this.opts.type}' is not a supported message type. The available options are:\n- alert\n- alert-bleed\n- alert-inner\n- notice\n- notice-bleed\n- notice-inner'`);
		}
	}
